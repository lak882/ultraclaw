...
Can be nested to group activities logically.
...
<call name='Send to Lab' target='LabSystemOperation' async='0' timeout='30'>
  <request type='EnsLib.HL7.Message' value='request'/>
  <response type='EnsLib.HL7.Message' value='context.labResponse'/>
</call>
```
...
<call name='Validate Order' target='ValidationService' async='0'>
  <request type='MyPkg.Msg.ValidateRequest' value='callrequest'>
  ...
  <response type='MyPkg.Msg.ValidateResponse' value='callresponse'>
...
**Note**: When using inline `<assign>` inside `<request>`, the `value` attribute of `<request>` should be `callrequest` (a new temporary object is created). When using inline `<assign>` inside `<response>`, the `value` attribute of `<response>` should be `callresponse`.
...
| Heavy ObjectScript processing, complex loops | Code-based process (`Ens.BusinessProcess`) |
| External API calls with complex retry logic | Code-based process |
| Performance-critical high-throughput path | Code-based process |
...
**Primary approach**: Use the full union group path to access OBR:
...
The path `ORCgrp(1).OBRuniongrp.OBRunion.OBR:FieldName` navigates through the union type correctly.

**Fallback** (if the full path doesn't resolve): Use segment-level access in `<code>` blocks:

```objectscript
; Loop through segments to find OBR
Set segCnt = request.SegCount
For i=1:1:segCnt {
  Set segData = request.GetSegmentAt(i)
  If $IsObject(segData) && (segData.GetValueAt(0) = "OBR") {
    Set tValue = segData.GetValueAt("UniversalServiceIdentifier.Identifier")
    Quit
  }
}
```

Always try the full group path first. Only fall back to segment-level access if it returns empty.
...
4. **Check the trace** to verify the BPL execution path:
   ```bash
   /usr/local/InterSystems/IRISHealth/bin/irispython .claude/skills/interclaw/scripts/documents/run_query.py --server <server> --namespace <ns> \
