...
<Pkg>.BPL.<Name>Process                       — BPL processes (visual/XML) ← DEFAULT for all orchestration
      e.g., <Pkg>.BPL.LabResultProcess
            <Pkg>.BPL.ADTOrchestrator
<Pkg>.BP.<Name>Process                        — code-based processes (RARE — only when BPL insufficient)
      e.g., <Pkg>.BP.HeavyStringProcessor
...
HSCUSTOM.ADTFeed.BPL.ADTOrchestrator          — BPL processes (default)
