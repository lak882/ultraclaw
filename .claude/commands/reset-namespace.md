...
3. Check if namespace exists:
   ```bash
   cd "/usr/local/InterSystems/interop-agent-orchestrator" && /usr/local/InterSystems/IRISHealth/bin/irispython .claude/skills/interclaw/scripts/infrastructure/manage_namespace.py --server <server> --exists <NAME>
   ```
