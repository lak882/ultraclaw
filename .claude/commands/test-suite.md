...
Steps:
1. Parse "$ARGUMENTS" — package prefix (required), `--to <configItem>`, `--wait <seconds>` (default 2), `--test-dir <dir>` (default tests/).
2. Build HTTP service URL from `--to` config item, or auto-detect from active production.
3. Run:
   ```bash
   cd "/usr/local/InterSystems/interop-agent-orchestrator" && /usr/local/InterSystems/IRISHealth/bin/irispython .claude/skills/interclaw/scripts/hl7/test_suite.py --server <server> --namespace <ns> --package <package> --url <url> [--test-dir <dir>] [--wait <wait>]
   ```
