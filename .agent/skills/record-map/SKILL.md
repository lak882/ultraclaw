...
| Verify target class got properties | `run_sql`: `SELECT Name, Type FROM %Dictionary.CompiledProperty WHERE parent = 'MyApp.Record.Foo' AND Name NOT %STARTSWITH '%'` |
...
- Compilation succeeding does NOT mean the target class generated. Verify with the SQL above after every push.
...
7. Push with `create_class` / `write_class`, then run the SQL above to verify target generation.
8. If target is empty, force with `EnsLib.RecordMap.Generator.GenerateObject` via `exec:`.
